...
class PetSCNView: SCNView {
    ...
    override func mouseUp(with event: NSEvent) {
        if !isDragging {
            ...
            let hits = hitTest(loc, options: nil)
            if !hits.isEmpty {
                let isHead = hits.contains { hit in isHeadNode(hit.node) }
                if isHead {
                    onClickHead?()
                } else {
                    onClickModel?()
            ...
            // hits empty = clicked empty space, don't focus
        }
        isDragging = false
    }
...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        scnView.onClickHead = { [weak self] in
        ...
        // Gesture monitors — only when focused, don't block desktop
        ...
        // Click anywhere else → unfocus model
